#!/bin/bash
# Copyright (c) 2013 The MLCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete MlCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no MLCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old MLCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected MLCoin 0.7 datadir."
    ;;
  3)
    echo "Detected MLCOin pre-0.8 datadir."
    ;;
  4)
    echo "Detected MLCoin 0.8 datadir."
    ;;
esac
